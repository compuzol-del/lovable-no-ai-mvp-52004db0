...
// 1. Whale consensus (unique wallets)
...
// 2. Capital deployed (log scale)
function scoreCapital(totalUsd: number): number {
  if (totalUsd <= 0) return 0;
  // log10($1K)=3 -> 60, log10($10K)=4 -> 80, log10($100K)=5 -> 100
  const v = Math.log10(totalUsd) * 20;
  return Math.max(0, Math.min(100, v));
}

// 3. Freshness (minutes since last buy)
...
// 4. Drift score (lower drift = better entry)
...
// 5. Price agreement (std dev across whale buys, USD-weighted)
function scorePriceAgreement(prices: number[], weights: number[]): number {
  ...
  const variance =
    prices.reduce((s, p, i) => s + weights[i] * (p - mean) ** 2, 0) / totalW;
  const std = Math.sqrt(variance);
  // std=0.02 -> 100, std=0.10+ -> 30
  if (std <= 0.02) return 100;
  if (std >= 0.10) return 30;
  return 100 - ((std - 0.02) / 0.08) * 70;
}

// 6. Burst (time window between first & last buy)
...
export const Route = createFileRoute("/api/public/hooks/compute-signals")({
  server: {
    handlers: {
      POST: async () => {
        ...
        const { data: rows, error } = await supabaseAdmin
          .from("trade_alerts")
          .select(
            "wallet_address,wallet_label,condition_id,asset,outcome,title,price,usdc_size,ts,side",
          )
          .gte("ts", since)
          .eq("side", "BUY")
        ...
        for (const g of groups.values()) {
          ...
          const weightedSum = g.rows.reduce(
            (s, r) => s + Number(r.price) * Number(r.usdc_size),
            0,
          );
          ...
          const driftPct =
            currentPrice != null ? ((currentPrice - avgPrice) / avgPrice) * 100 : 0;
          ...
          signalsToInsert.push({
            ...
            price_std: Number(priceStd.toFixed(4)),
            burst_minutes: Number(burstMinutes.toFixed(1)),
