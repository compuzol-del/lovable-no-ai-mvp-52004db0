...
export const Route = createFileRoute("/api/public/hooks/real-execute")({
  server: {
    handlers: {
      POST: async () => {
        // 1. Load config
        ...
        for (const p of openPos || []) {
          ...
          // Whale-reversal exit (more sensitive: ≥1 sell)
          ...
          if (exitReason) {
            ...
            // Apply fee only on winning side (Polymarket charges ~2% on winnings)
        ...
        // 4. OPEN: scan recent STRONG_BUY signals
        if (cfg.enabled && !halted) {
          // Check max_open_total
          ...
          if ((openCount ?? 0) >= Number(cfg.max_open_total)) {
            ...
            for (const s of signals || []) {
              ...
              const { data: market } = await supabaseAdmin
                .from("markets")
                .select("volume,liquidity,event_id")
                .eq("condition_id", s.condition_id)
                .maybeSingle();

              if (market) {
                if (market.volume != null && Number(market.volume) < Number(cfg.min_market_volume_usd)) {
                  skipped.push({ condition_id: s.condition_id, why: `low volume ${market.volume}` });
                  continue;
                }
                if (market.liquidity != null && Number(market.liquidity) < Number(cfg.min_market_liquidity_usd)) {
                  skipped.push({ condition_id: s.condition_id, why: `low liquidity ${market.liquidity}` });
                  continue;
                }
                // Per-event cap
                if (market.event_id) {
                  const { count: eventOpen } = await supabaseAdmin
                    .from("real_positions")
                    .select("id", { count: "exact", head: true })
                    .eq("status", "OPEN")
                    .eq("event_id", market.event_id);
                  if ((eventOpen ?? 0) >= Number(cfg.max_open_per_event)) {
                    skipped.push({ condition_id: s.condition_id, why: `event cap (${eventOpen}/${cfg.max_open_per_event})` });
                    continue;
                  }
                }
              }
              ...
              if (refPrice > 0) {
                ...
                if (slippagePct > Number(cfg.max_slippage_pct)) {
                  skipped.push({ condition_id: s.condition_id, why: `slippage ${slippagePct.toFixed(2)}%` });
                  continue;
                }
              }
              ...
              const { error: insErr, data: ins } = await supabaseAdmin
                ...
                .insert({
                  ...
                  market_volume_usd: market?.volume ?? null,
                  market_liquidity_usd: market?.liquidity ?? null,
                  order_id: orderId,
                  dry_run: !!cfg.dry_run,
                })
                .select("id")
              ...
              if (insErr) {
                ...
                opened.push({ id: ins?.id, score: s.score, sizeUsd, entry, dry_run: !!cfg.dry_run });
                currentOpen += 1;
              }
            }
          }
